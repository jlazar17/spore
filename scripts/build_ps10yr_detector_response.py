...
(via the existing TOML config), averages across seasons, and writes a
single HDF5 file in the spore hierarchical format::
...
-----
    python scripts/build_ps10yr_detector_response.py \\
        --toml  resources/configs/ps10yr_response.toml \\
        --output resources/configs/ps10yr_detector_response.h5
"""
...
def _resolve(path: str, base_dir: str) -> str:
    ...
    if os.path.exists(resolved):
        return resolved
    return path  # fall back to CWD-relative
...
def build(toml_path: str, output_path: str) -> None:
    ...
    # ── Locate morphology configs ────────────────────────────────────────────
    _RESPONSE_KEYS = {"effective_area", "psf", "smearing"}
    is_flat = bool(_RESPONSE_KEYS & set(cfg.keys()))
    morph_configs = {"track": cfg} if is_flat else {
        k: v for k, v in cfg.items() if isinstance(v, dict)
    }

    with h5py.File(output_path, "w") as hf:
        for morph, mcfg in morph_configs.items():
            grp = hf.require_group(morph)

            # ── Effective area ───────────────────────────────────────────────
            ea_cfg = mcfg.get("effective_area", {})
            if ea_cfg.get("type") == "dataverse_csv":
                irf_dir = _resolve(ea_cfg["irf_dir"], toml_dir)
                stems   = _irf_stems(ea_cfg["seasons"])
                ...
                print(f"  Wrote {morph}/effective_area  "
                      f"(shape {aeff_avg.shape}, "
                      f"max={aeff_avg.max():.3e} cm²)")

            # ── Joint smearing ───────────────────────────────────────────────
            sm_cfg = mcfg.get("smearing", {})
            if sm_cfg.get("type") == "dataverse_csv":
                irf_dir = _resolve(sm_cfg["irf_dir"], toml_dir)
                stems   = _irf_stems(sm_cfg["seasons"])
                ...
                print(f"  Wrote {morph}/smearing  "
                      f"(frac shape {frac_avg.shape}, "
                      f"averaged over {n} IRF file(s))")
...
def _parse_args(argv=None):
    ...
    p.add_argument(
        "--toml", default="resources/configs/ps10yr_response.toml",
        help="Path to the spore detector-response TOML (default: resources/configs/ps10yr_response.toml)",
    )
    p.add_argument(
        "--output", default="resources/configs/ps10yr_detector_response.h5",
        help="Output HDF5 path (default: resources/configs/ps10yr_detector_response.h5)",
    )
...
if __name__ == "__main__":
    args = _parse_args()
    print(f"Reading TOML : {args.toml}")
    print(f"Writing HDF5 : {args.output}")
    build(args.toml, args.output)
