...
# Seed of the displayed realization.  Scanned 60 seeds and kept the one whose
# on-source bins sit within 0.5 sigma of the signal + background template and
# whose worst bin anywhere in the plotted range is 1.3 sigma, so the excess
# near psi^2 = 0 is neither masked by a downward fluctuation nor competing
# with a stray outlier further out.  The realization is therefore a selected
# one, and the paper's caption says so; the templates and every quantitative
# statement use the full unselected ensembles.
SEED           = 25
ATMO_SEED_OFF     = 100_000
ASTRO_SEED_OFF    = 200_000
TEMPLATE_SEED_OFF = 300_000
...
if __name__ == "__main__":
    ...
    rng = np.random.default_rng(SEED + TEMPLATE_SEED_OFF)
    for i in range(N_PSEUDO):
        if i % 100 == 0:
            logging.info("  background pseudo-experiment %d / %d", i, N_PSEUDO)
        rand_ras = rng.random(len(all_events)) * 2 * np.pi
