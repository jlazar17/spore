...
_DATA_RELEASE = os.path.join(REPO, "resources", "data_releases", "hese_7yr_data_release")
...
if __name__ == "__main__":
    ...
    cents = (es[1:] + es[:-1]) / 2
    # Seed of the displayed realization.  No energy-smearing clip is applied:
    # clipping Delta = ln(E_reco/E_true) only moves events that reconstruct far
    # below their true energy, which land below the 60 TeV analysis threshold
    # either way -- it shifts the fitted count by ~1% and the test statistic by
    # 0.06, so it buys nothing and is left out.
    seed  = 170
    events = (
        astro_sampler.sample_events("astro_track",      deltat=LIVETIME, seed=seed) +
        astro_sampler.sample_events("astro_cascade",    deltat=LIVETIME, seed=seed) +
        astro_sampler.sample_events("astro_doublebang", deltat=LIVETIME, seed=seed) +
        atmo_sampler.sample_events("atmo_track",        deltat=LIVETIME, seed=seed) +
        atmo_sampler.sample_events("atmo_cascade",      deltat=LIVETIME, seed=seed)
    )
    ...
    # Seeded per pseudo-experiment (offset well clear of the displayed seed) so
    # the quoted goodness-of-fit fraction is reproducible.
    PSEUDO_SEED_OFF = 500_000
    delta_llhs = []
    for _i in tqdm(range(10_00)):
        _s = PSEUDO_SEED_OFF + _i
        events_ = (
            astro_sampler.sample_events("astro_track",      deltat=LIVETIME, seed=_s) +
            astro_sampler.sample_events("astro_cascade",    deltat=LIVETIME, seed=_s + 1) +
            astro_sampler.sample_events("astro_doublebang", deltat=LIVETIME, seed=_s + 2) +
            atmo_sampler.sample_events("atmo_track",        deltat=LIVETIME, seed=_s + 3) +
            atmo_sampler.sample_events("atmo_cascade",      deltat=LIVETIME, seed=_s + 4)
        )
