...
if __name__ == "__main__":
    ...
    for idx, dec_rad in enumerate(DECS_RAD):
        ...
        for _ in range(N_RA):
            ...
            effas[0, idx, :] += icecube.response.effective_area["track"](np.full(len(ES), ic_local.zenith),  ES)
            effas[1, idx, :] += km3net.response.effective_area["track"](np.full(len(ES), km3_local.zenith), ES)
    effas /= N_RA
