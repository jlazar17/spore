...
class Flux:
    ...
    def __call__(self, nu: Neutrino, e: float, dec: Optional[float] = None) -> float:
        ...
        Returns:
            Differential flux in eV^{-1} cm^{-2} s^{-1} sr^{-1}.
        """
    ...
    @classmethod
    def from_config(cls, config: Dict) -> 'Flux':
        ...
        if has_powerlaw:
            ...
            norm = raw_norm / units.GeV / units.cm**2 / units.sec
            if config["gamma"]==1:
